...
export class RenderManager {
  ...
  setViewport(snapshot = {}) {
    ...
    if (
      ...
      prev.devicePixelRatio === nextViewport.devicePixelRatio
    ) {
      return;
    }
  ...
  setGrid({ width, height, tileSize } = {}) {
    ...
    const nextTileSize = Math.max(1, Math.round(toFiniteNumber(tileSize, this.state.tileSize)));
    const prevGrid = this.state.gridBounds;
    const prevTileSize = this.state.tileSize;
    if (
      prevGrid.width === nextGridBounds.width &&
      prevGrid.height === nextGridBounds.height &&
      prevTileSize === nextTileSize
    ) {
      return;
    }
    this.state.gridBounds = nextGridBounds;
    this.state.tileSize = nextTileSize;
    this.#emit();
  }
